...
public class CDataPackString {
	...
	public void SetData(string key,object val) {		
		...
		if (val is string) {
			...
			Debug.LogError("[SaveLib] SetData Syntax Error");
		}
	...
	public string EncodeDataPackString() {
		...
		foreach (KeyValuePair<string,object> data in dataList) {
			...
			if (data.Value is string) {
				...
				Debug.LogError(string.Format("[SaveLib] EncodeDataPackString Syntax Error {0},{1}",data.Key,data.Value));
			}

		}

		if (DebugLog) {
			Debug.Log ("[SaveLib] " + rtnString);
		}
